...
try:
    try:
        try:
            ...
            import sys
            import os
            sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'sherlock_project')))
            from . import import_error_test_var  # noqa: F401
    except ModuleNotFoundError:
        import sys
        import os
        sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'sherlock_project')))
...
def sherlock(
    ...
    for social_network, net_info in site_data.items():
        # Ensure net_info is an instance of SiteInformation
        if isinstance(net_info, SiteInformation):
            results_site = {
                "url_main": net_info.url_home,  # Access the attribute directly
            }

            # Check if the url_username_format contains the placeholder
            if "{username}" not in net_info.url_username_format:
                raise ValueError(
                    f"url_username_format for site '{social_network}' is missing the '{{username}}' placeholder."
                )

            # Construct the URL for the username
            url = net_info.url_username_format.format(username=username)
            results_site["url_user"] = url

            # Add this site's results into the final dictionary
